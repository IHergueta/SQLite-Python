...
try:
    ...
    cur.execute(
        "create table productos(id integer PRIMARY KEY, nombre text, precioventa real, preciocoste real, stockactual integer)")
    con.commit()
...
try:
    ...
    cur.execute(
        "create table stock(id integer PRIMARY KEY, cdgproducto integer, almacen text, stock integer, FOREIGN KEY(cdgproducto) REFERENCES productos(id))")
    con.commit()
...
try:
    ...
    cur.execute(
        "create table ventas(id integer PRIMARY KEY, cdgproducto integer, cantidad integer, ciudad text,importe real, precio real, FOREIGN KEY(cdgproducto) REFERENCES productos(id))")
    con.commit()
